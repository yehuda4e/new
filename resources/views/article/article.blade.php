<article class="ui stacked segments">
    <div class="ui segment">
        ...
        <h3 class="header" style="display: flex">
            ...
            <form action="" onclick="if(!confirm('Are you sure?')) return false;" method="post">
                {{ csrf_field() }}
                {{ method_field('delete') }}
                <button class="ui tiny red button"><i class="fa fa-trash"></i> Delete</buttona>
            </form>
            @endif
        </h3>
    </div>
    ...
    <div class="ui secondary segment" style="display: flex">
        <div style="flex: 1">
            ...
            &#x1F550; </i> <time>{{ $article->created_at->diffForHumans() }}</time> 
            &#x1F4AC; </i> <a href="/{{ $article->slug }}#comments">{{ $article->comments()->count() }} {{ str_plural('comment', $article->comments()->count()) }}</a>
